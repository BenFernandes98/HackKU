...
func main() {
	...
	r.HandleFunc("/hate", hate).Methods(http.MethodGet)
	r.HandleFunc("/cornhub/{rating}", cornhub).Methods(http.MethodGet)
...
// New function
func hate(w http.ResponseWriter, r *http.Request) {
	w.WriteHeader(http.StatusOK)
	fmt.Fprintf(w, "%s", "<html><head>CornHub</head><body><h1>CORNHUB</h1>I hate Go already</body></html>")
}

// Cornhub function
func cornhub(w http.ResponseWriter, r *http.Request) {
	rating := mux.Vars(r)["rating"]
	w.WriteHeader(http.StatusOK)
	fmt.Fprintf(w, "%s", "Your compatability rating is: " + rating + "!\n")
